...
import com.zhifei.cloud.filter.JwtAuthFilter;
import com.zhifei.cloud.filter.JwtVerifyFilter;
import com.zhifei.cloud.handler.*;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    /**
     * 自定义配置，默认的登录url为/login（post），退出为/logout（post/get），均不需要登录就可以访问
     */
    @Override
    protected void configure(HttpSecurity http) throws Exception {
        JwtAuthFilter jwtAuthFilter = new JwtAuthFilter(super.authenticationManager(), jwtProperties.getRsaPrivateKey(), "/auth/login");
